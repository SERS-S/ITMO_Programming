...
public class CoordinatesCheck 
{
    public static boolean coordinatesCheck(String input)
    {
        if ((input.split("_").length == 2))
        {
            ...
            if (coordinates_flag && (Integer.parseInt(input.split("_")[0]) <= Integer.MAX_VALUE) && (Integer.parseInt(input.split("_")[0]) >= Integer.MIN_VALUE) && (Float.parseFloat(input.split("_")[1]) <= Float.MAX_VALUE) && (Float.parseFloat(input.split("_")[1]) >= Float.MIN_VALUE))
            {
                System.out.println(ColorText.ColorGREEN("coordinates получены"));
                System.out.println(ColorText.ColorYELLOW("Теперь введите health: (пример 4.3, health > 0)"));
                return true;
            }
            else
            {
