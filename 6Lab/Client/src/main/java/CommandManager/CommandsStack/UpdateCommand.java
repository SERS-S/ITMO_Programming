...
public class UpdateCommand implements Command {
  ...
  public NetworkRequestaGenericPackage execute(
    ...
    if ((line.split(" ").length == 3) && (line.split(" ")[0].equals("update"))) {
      ...
      if (Arrays.asList(
              "name", "coordinates", "health", "loyal", "achievements", "weaponType", "chapter")
          .contains(line.split(" ")[2].trim())) {
        System.out.println(
            "  > "
                + ColorText.ColorGREEN("Введите новое значение поля ")
                + line.split(" ")[2].trim()
                + ColorText.ColorGREEN(" элемента:"));
    ...
    NetworkRequestaGenericPackage packData = new NetworkRequestaGenericPackage("update", flag);
    if (flag) {
      packData.getMainData(
          line.split(" ")[1], name, coordinates, health, loyal, achievements, weaponType, chapter);
    }
    return packData;
  }
}
