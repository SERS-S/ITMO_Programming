package Collection.InitialiseArray.FormatsDecoders;
  ...
  public static Vector<String[]> readerToData(BufferedReader reader) throws IOException {
...

    {
      ...
      for (int ii = 2; ii < lines.size(); ++ii) {
        if (control_count != lines.get(ii).length) {
          throw new IOException("Файл с данными не корректен (разное кол-во данных в строках)");
        }
      }
    }
