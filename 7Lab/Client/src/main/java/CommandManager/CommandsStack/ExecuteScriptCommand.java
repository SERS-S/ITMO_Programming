...
public class ExecuteScriptCommand implements Command {
  ...
  public NetworkRequestGenericPackage execute(String line, CommandManager commandManager, Scanner scan) {
    ...
    if ((line.split(" ").length == 2) && (line.split(" ")[0].trim().equals("execute_script")) && (!line.split(" ")[1].trim().equals(""))) {
    ...
    NetworkRequestGenericPackage packData = new NetworkRequestGenericPackage("dataPackage", "execute_script", flag);
    if (flag) {
