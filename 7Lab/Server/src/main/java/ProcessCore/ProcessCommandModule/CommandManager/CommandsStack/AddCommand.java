...
public class AddCommand implements Command {
  ...
  public ArrayList<String> execute(CollectionManager collectionManager, NetworkRequestGenericPackage packOne, CommandManager commandManager) {
    ...
    try {
      ...
      statementConnection.addNewElement(
        String.valueOf(SpaceMarine.generateId(
          packOne.getDataStrings()[0], 
        ...
        packOne.getDataStrings()[1], 
        packOne.getDataStrings()[2].replace("{", "(").replace("}", ")").replace(";", ","),
        Timestamp.from(timeNow.toInstant()),
        packOne.getDataStrings()[3], 
        packOne.getDataStrings()[4], 
        packOne.getDataStrings()[5], 
        packOne.getDataStrings()[6], 
        packOne.getDataStrings()[7].replace("{", "(").replace("}", ")").replace(";", ","),
        packOne.getClientUser().getLogin());
