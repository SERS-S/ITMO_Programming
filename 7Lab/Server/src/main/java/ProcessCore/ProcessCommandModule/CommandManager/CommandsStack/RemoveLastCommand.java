...
public class RemoveLastCommand implements Command {
  ...
  public ArrayList<String> execute(CollectionManager collectionManager, NetworkRequestGenericPackage packOne, CommandManager commandManager) {
    ...
    try {
      ...
      if (collectionManager.getCollection().get(collectionManager.getCollection().size() - 1).getLineData().split(" ")[9].equals(packOne.getClientUser().getLogin())) {

        statementConnection.removeByIdEelement(collectionManager.getCollection().get(collectionManager.getCollection().size() - 1).getLineData().split(" ")[0]);
        ...
        responceStrings.add("GREEN#" + "  > " + "Последний элемент коллекции с id: " + packOne.getDataStrings()[0] + " успешно удален");
      } else {
